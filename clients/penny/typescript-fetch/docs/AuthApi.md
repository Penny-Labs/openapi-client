...
| [**listSessionFeatures**](AuthApi.md#listsessionfeatures) | **GET** /v1/auth/features | List resolved feature flags for the authenticated user |
...
> FeatureListResponse listSessionFeatures()
...
import type { ListSessionFeaturesRequest } from '@penny-labs/openapi-penny-client';
...
async function example() {
  ...
  try {
    const data = await api.listSessionFeatures();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
